...
class ReplayMemory:
    ...
    def push(self, state: np.ndarray, action: np.ndarray, reward: float, done: bool, next_state: np.ndarray):
    ...
    def get_batch(self, batch_size: int) -> Tuple[ft, torch.LongTensor, ft, ft, ft]:
        ...
        return ft(splits[0]), torch.LongTensor(splits[1]), ft(splits[2]), ft(splits[3]), ft(splits[4])
